...
export function useRendezVous(dateFiltre = null) {
  ...
  async function modifierRdv(id, champs) {
    const { data, error } = await supabase
      .from('rendez_vous')
      .update(champs)
      .eq('id', id)
      .select(`*, patients(nom, prenom)`)
    if (!error) setRendezVous(prev => prev.map(r => r.id === id ? data[0] : r))
    return { data, error }
  }

  return { rendezVous, loading, erreur, charger, ajouterRdv, modifierStatut, modifierRdv, supprimerRdv }
}
